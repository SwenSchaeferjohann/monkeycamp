import React, { Component, createElement} from 'react';
import { Row, Col, Form, Card, Button} from 'react-bootstrap';
 
// import CampaignHeader from './CampaignHeader'
// import { ButtonToolbar, Button } from 'react-bootstrap'
class TheFeedBackForm extends Component {
  constructor(props) {
    super(props);
    this.state = {  }
  }
  render() { 
    return ( 
      <div >
        <Card className="feedback-card" style={{ width: '18rem' }}>
        
        <Card.Body>
          <Card.Title>

          <Form.Label className="feedback-label-big"> 
              <div>

                Let us know!
              </div>
            </Form.Label>


            <Form.Label className="feedback-label"> 
              <div>

                Did you meet us at Disrupt 2019?
              </div>
            </Form.Label>
          </Card.Title>
          <Card.Text>
            
          </Card.Text>
          <Row>
            <div className ="yes-no-wrapper">
              <Button className="yes-btn" variant="outline-dark">Yes </Button>
              <Button className="no-btn" variant="outline-dark">Nope</Button>
            </div>
          </Row>
        </Card.Body>
        </Card>

      </div>
     );
  }
}
 
export default TheFeedBackForm;
